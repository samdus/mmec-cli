...
[#installation]
Récupérer le zip de la dernière version disponible dans le répertoire "link:./distributions[distributions]" et dézipper le contenu. Vous pourrez ensuite utiliser le script mmec-cli sous Unix  ou mmec-cli.bat sous Windows. Ces scripts se trouvent dans le répertoire /bin du zip.
